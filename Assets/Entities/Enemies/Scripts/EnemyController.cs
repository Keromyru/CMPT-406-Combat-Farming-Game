...
public class EnemyController : MonoBehaviour, IEnemyControl, ITakeDamage
{
    ...
    private void FixedUpdate() {
        ...
        if (!isWaiting  && attackTarget != null ) {
            if( Vector2.Distance(this.GetComponent<BoxCollider2D>().ClosestPoint(attackTarget.transform.position), attackTarget.GetComponent<BoxCollider2D>().ClosestPoint(this.transform.position)) < myEnemyData.attackRange ||
